...
--> 계정 명이 언급되는 상황에서는 이 구문 필수적으로 진행!
-- SYS가 KH계정에 VIEW생성 권한 부여
GRANT CREATE VIEW TO kh;
--여기서부터 다시 kh.
--아래 VIEW 구문 다시 실행
CREATE VIEW V_EMP
AS SELECT * FROM EMPLOYEE; -- 가상테이블 . 실체 없음
...
--VIEW 조회 결과로 보이는 컬럼명을 이용해야함.
SELECT * FROM V_EMP
WHERE 직급명 ='대리'
ORDER BY 이름;
--------------------------------------------------------------------------
/*VIEW를 이용해서 DML 사용하기 + 문제점 확인 */
-- DEPARTMENT TABLE을 복사한 DEPT_COPY2 (테이블로 생성)
CREATE TABLE DEPT_COPY2 AS SELECT * FROM DEPARTMENT;

--DEPT_COPY2 테이블에서 DEPT_ID, LOCATION_ID컬럼만 이용해서 
-- V_DCOPY2 VIEW 생성
...
--VIEW가 원본테이블에 비해 1가지 컬럼이 적음.


--V_DCOPY2 VIEW를 이용해서 INSERT 수행
INSERT INTO V_DCOPY2
VALUES ('D0','L2');
--가상 테이블인 VIEW인데도 , INSERT 성공!! ( DML 사용가능)
SELECT* FROM V_DCOPY2;
--D0 L2 값 잘 들어가 있음.
SELECT * FROM DEPT_COPY2;
--> VIEW에 INSERT 수행했지만 VIEW생성시 사용한 원본테이블에 값이 함께 INSERT됨을 확인!!!!**



--> 모든 컬럼 값이 INSERT 된것이 아니라
-- VIEW 생성할때 사용된 컬럼에만 데이터가 삽입 되었고, 
--사용되지 않은 컬럼 (DEPT_TITLE)에는 NULL이 들어감.

--> NULL은 DB의 무결성을 약하게 만드는 주요원인.
-->가능하면 의도되지 않은 NULL은 존재하지 않게 한다.
...
--SQL Error [2289] [42000]: ORA-02289: 시퀀스가 존재하지 않습니다.
-->CURRVAL의 정확한 의미는 가장 최근 호출된 NEXTVAL의 값을 반환함을 뜻함.
...
SELECT SEQ_TEST_NO.CURRVAL FROM DUAL;-- 가장 최근에 반환된 NEXTVAL은 100

--NEXTVAL를 호출 할때 마다 
--INCREMENT BY에 작성된 수만큼 증가하는지 확인
...
INSERT INTO TB_TEST
VALUES (SEQ_TEST_NO.NEXTVAL,'짱구');--115

INSERT INTO TB_TEST
VALUES (SEQ_TEST_NO.NEXTVAL,'철수');--120

INSERT INTO TB_TEST
VALUES (SEQ_TEST_NO.NEXTVAL,'유리');--125
...
---------------------------------------------------------------
--UPDATE에서 시퀀스 사용하기
-- '짱구'의 PK 컬럼값을 SEQ_TEST_NO 시퀀스의 다음 생성값으로 변경하기 

UPDATE TB_TEST
SET TEST_NO = SEQ_TEST_NO.NEXTVAL
WHERE TEST_NAME = '짱구'; -- 이걸 수행할때마다 짱구값이 130,135,140,145,150되었다가
--150이상은 호출 X 
--SQL Error [8004] [72000]: ORA-08004: 시퀀스 SEQ_TEST_NO.NEXTVAL exceeds MAXVALUE은 사례로 될 수 없습니다

--즉, MAX VALUE 150 보다 더 증가할수 없다.

SELECT * FROM TB_TEST; -- 짱구 150확인 가능.

----------------------------------------------------------
--SEQUENCE 변경 (ALTER)
-->STRAT WITH 빼고 모두 변경 가능!!
...
--SEQ_TEST_NO의  MAXVALUE값을 200 으로 수정
...
SELECT * FROM TB_TEST; -- 짱구 200확인 가능.

----------------------------------------------------------------------------
--VIEW, SEQUENCE 삭제

--VIEW : V_DCOPY2
--SEQUENCE : SEQ_TEST_NO
...
FROM EMPLOYEE;
--ROWID : 오라클 에서 각 행의 고유한 주소를 나타내는 가상 컬럼 (물리적 주소)
-- 인덱스가 ROWID 저장함.
--인덱스는 컬럼의 값과 해당행의 ROWID를 매핑해서 저장함.
--인덱스 역할 : 컬럼값을 통해 해당행의 ROWID를 빠르게 찾아낼 수 있다.

--현재 사용자에 생성된 INDEX 목록 조회
...
--> EMPLOYEE 테이블의 EMP_NAME컬럼에 대해 빠른 검색이 가능
...
--TB_IDX_TEST 테이블에 샘플데이터 100만개 삽입 (PL/SQL사용)
--* PL/SQL 이란? 
--오라클 데이터베이스에서 사용하는 절차적 확장 언어.
--변수, 조건문(IF/CASE),반복문 (LOOP,WHILE)사용 가능
BEGIN 
	FOR I IN 1..1000000 --I라는 변수는 1부터 100만까지 반복한다는 뜻
	LOOP 
	INSERT INTO TB_IDX_TEST VALUES (I,'TEST'||I)	;
	END LOOP;
COMMIT;--루프안에서 했던 모든 삽입작업을 커밋하겠다.
END;

SELECT COUNT(*)FROM TB_IDX_TEST;--100만개 행이 삽입됨을 확인

--인덱스를 사용해서 검색하는 방법
--WHERE절에 INDEX가 적용된 컬럼을 언급하기


/*인덱스 사용 X */
SELECT * FROM TB_IDX_TEST
WHERE TEST_ID ='TEST500000';--0.018S
