...
---------------------------------------------------------------------
--EMPLOYEE 테이블에서 이메일에 언더바앞에 글자가 세글자인 사람만 이름 이메일 조회
SELECT EMP_NAME,EMAIL
FROM EMPLOYEE
WHERE EMAIL LIKE '___%';--어떻게 하는지 모르겠다. 

--ESCAPE 문자 :LIKE 에서 사용하는 _는 패턴일 뿐 내가 찾고싶은 문자 _ 가 아님.
--ESCAPLE문자뒤에 작성된 _는 일반 문자로 탈출한다는 뜻
-- # , ^ 둘중에 하나 사용하면 됨 아무거나.
...
------------------------------------------------------------------------
--연습문제
--EMPLOYEE 테이블에서 이메일'_'앞이 네글자 이면서 부서코드가 D9또는 D6이고, 입사일이 1990-01-01~ 2000-12-31 사이이고,
-- 급여가 270만원 이상인 사원의 사번, 이름, 이메일,부서코드,입사일 급여 조회
...
-- 8. OR
   ...
   * IN 연산자
   * 
   * 비교하려는 값과 목록에 작성된 값중 일치하는 것이 있으면 조회하는 연산자
   * 
   * [작성법]
 ...
 --EMPLOYEE 테이블에서 부서코드가 D1,D6,D9인 사원의 사번이름부서코드 조회
...
SELECT EMP_ID,EMP_NAME,DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE NOT IN ('D1','D6','D9');-- 12명(DEPT_CODE NULL미포함)
  
  
SELECT EMP_ID,EMP_NAME,DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE NOT IN ('D1','D6','D9')
OR DEPT_CODE IS NULL; -- 14명 (NULL두명 포함)
...
--EMPLOYEE테이블에서 보너스가 있는 사원의 이름, 보너스 조회

SELECT EMP_NAME, BONUS
FROM EMPLOYEE
WHERE BONUS IS NOT NULL;--보너스 받는 사원 9명


SELECT EMP_NAME, BONUS
FROM EMPLOYEE
WHERE BONUS IS NULL;--보너스 못받는 사원14명
...
--EMPLOYEE테이블에서 급여 오름차순으로 사번 이름 급여 조회
...
--EMPLOYEE테이블에서 급여가 200만원 이상인 사람의 사번 이름 급여 조회
--단 급여 내림차순
SELECT EMP_ID,EMP_NAME,SALARY
FROM EMPLOYEE
